...
#include <iostream>
#define datasize 100
extern "C" void BicubicScale_host(int *pixel, int Width, int Height);
...
__global__ void BicubicScale(int *In, int *Out, int Width, int Height)
{
	int window[9];
...
extern "C" void BicubicScale_host(int *pixel, int Width, int Height)
{
	...
	BicubicScale << <dimGrid, dimBlock >> > (pixelIn, pixelOut, Width, Height);
